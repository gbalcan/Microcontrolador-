...
float le_pulso(void)
{
	...
	TR1 = 0;
	TF1 = 0;
	th_t1 = (unsigned int)TH1*256 + (unsigned int)TL1;
	TL1 = 0;
	TH1 = 0;
	cont_t = cont_t + (float)th_t1/25000000;
	return (cont_t);
}

void int_tc1() __interrupt 3
{
	TF1 = 0;
	cont_t += 0.00262144;
...
void main()
{
	...
	{
		dist = le_hcsr04();
		printf_fast_f("\x01 pulsos=%8d", dist);
		printf_fast_f("\x02 CM=%12d", dist/58);
		delay_ms(300);
	}
}